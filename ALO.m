...
for i=1:N
antlion_position(i,:)=Initialize(lb,ub,dim);
ant_position(i,:)=Initialize(lb,ub,dim);
end
...
while t<Max_iter+1
    ...
    for i=1:size(ant_position,1)
        ...
        % create new poistion for each ant according to Elite antlion(RE)
        % and selected antlion(RA)                     %Eq(16) in paper
        R_A=Next_walk(Sorted_antlions(Rolette_index,:));                      
        R_E=Next_walk(Elite_antlion_position(1,:));
             
        ant_position(i,:)= CrossOver(R_A,R_E);      %Eq(17) in paper           
